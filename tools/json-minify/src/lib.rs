...
pub fn minify_json(input: &str) -> Result<String, JsonMinifyError> {
    let value: serde_json::Value = serde_json::from_str(input).map_err(JsonMinifyError::JsonParse)?;
    serde_json::to_string(&value).map_err(JsonMinifyError::JsonSerialize)
}
...
impl fmt::Display for JsonMinifyError {
...
impl std::error::Error for JsonMinifyError {}
...
mod tests {
    ...
    fn minifies_json() {
        ...
        let expected: serde_json::Value = serde_json::from_str(input).expect("input should parse");
        let actual: serde_json::Value = serde_json::from_str(&output).expect("output should parse");
    ...
    fn preserves_nested_structure() {
        let output = minify_json("{\"tools\":[\"json-pretty\",\"json-minify\"],\"meta\":{\"version\":1}}")
            .expect("conversion should succeed");
    ...
    fn rejects_invalid_json() {
        let error = minify_json("{invalid json}").expect_err("input should fail");
