...
pub fn minify_yaml(input: &str) -> Result<String, YamlMinifyError> {
    let value: serde_yaml::Value = serde_yaml::from_str(input).map_err(YamlMinifyError::YamlParse)?;
    serde_yaml::to_string(&value)
        .map(|output| output.trim_end().to_owned())
        .map_err(YamlMinifyError::YamlSerialize)
}
...
#[derive(Debug)]
pub enum YamlMinifyError {
    YamlParse(serde_yaml::Error),
    YamlSerialize(serde_yaml::Error),
}

impl fmt::Display for YamlMinifyError {
...
impl std::error::Error for YamlMinifyError {}
...
fn format_error(error: &YamlMinifyError, offending_value: &str) -> String {
...
#[cfg(test)]
mod tests {
    use super::minify_yaml;

    #[test]
    fn normalizes_yaml_document() {
        let output = minify_yaml("name: tool-store\nenabled: true\ncount: 3\n")
    ...
    fn rejects_invalid_yaml() {
        let error = minify_yaml("name: [invalid").expect_err("input should fail");
